...
importConfigs() {
    ...
    # chezmoi 用 config を配置
    #   chezmoi は起動時に config を参照するため事前配置しておく
    printf 'Configure chezmoi...\n'
    mkdir -p "$_CHEZMOI_CONFIG_DIR"
    cp "$_REPOS/chezmoi.toml" "$_CHEZMOI_CONFIG_DIR/chezmoi.toml"
    printf 'done\n\n'
...
postProcess() {
    ...
    # 構成管理を適用 (macOS 専用)
    printf 'Run Ansible...\n'
    ansible-playbook $_REPOS/native/macos/ansible.yml
    printf 'done\n\n'
}
