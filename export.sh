...
exportVSCodeConfig() {
    ...
    # User Config
    mkdir -p "$_ROOT/home/dot_vscode"
    cp -r "$HOME/.vscode/cspell.json" "$_ROOT/home/dot_vscode/cspell.json"

    # 設定ファイルを退避
    mkdir -p "$_ROOT/home/Library/vscode"
    for vscode_config in "$_VSCODE_USER_DIR"/*.json "$_VSCODE_USER_DIR"/*.code-workspace; do
        [ -f "$vscode_config" ] || continue
        cp "$vscode_config" "$_ROOT/home/Library/vscode/$(basename "$vscode_config")"
    done
