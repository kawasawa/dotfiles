...
exportDotfiles() {
    ...
    cp -r "$HOME/Library/Application Support/com.mitchellh.ghostty/config" "$_ROOT/configs/ghostty/config"
    printf 'done\n'

    printf 'Export Yazi config...\n'
    cp -r "$HOME/.config/yazi/" "$_ROOT/configs/yazi"
    printf 'done\n'
...
exportVSCodeConfig() {
    ...
    cp -r "$_VSCODE_USER_DIR/prompts/." "$_ROOT/configs/vscode/prompts/"
