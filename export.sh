...
exportClaudeCodeConfig() {
    ...
    mkdir -p "$_ROOT/configs/claude"
    cp -r "$HOME/.claude/settings.json" "$_ROOT/configs/claude/settings.json"

    # configs/claude/ に同名サブフォルダがあるものだけエクスポート
    for d in "$_ROOT/configs/claude"/*/; do
        [ -d "$d" ] || continue
        dir_name=$(basename "$d")
