...
alias diskinfo='smartctl -a /dev/disk0'            # ストレージ情報を表示
alias encb64='(){echo -n "$1" | base64}'           # Base64 エンコード
alias decb64='(){echo -n "$1" | base64 -d}'        # Base64 デコード
alias encurl='(){echo "$1" | nkf -WwMQ | tr = %}'  # URL エンコード
alias decurl='(){echo "$1" | nkf -w --url-input}'  # URL デコード
alias cmppdf='(){gs -sDEVICE=pdfwrite -dCompatibilityLevel=1.4 -dPDFSETTINGS=/default -dNOPAUSE -dQUIET -dBATCH -sOutputFile=$2 $1}'  # PDF を圧縮
alias cmpimg='(){convert -quality 80 $1 $2}'       # 画像を圧縮
alias img2ico='(){convert -define icon:auto-resize=128 $1 $2}'  # 画像を ico 形式に変換
